...
func (q *Queries) CreateTrip(ctx context.Context, pool *pgxpool.Pool, params spec.CreateTripRequest) (uuid.UUID, error) {
	...
	if err != nil {
		return uuid.UUID{}, fmt.Errorf("pgstore: faile to insert trip for CreateTrip: %w", err)
	}
	...
	if _, err := qtx.InviteParticipantsToTrip(ctx, participants); err != nil {
		return uuid.UUID{}, fmt.Errorf("pgstore: faile to insert participants for CreateTrip: %w", err)
	}

	if err := tx.Commit(ctx); err != nil {
		return uuid.UUID{}, fmt.Errorf("pgstore: faile to commit tx for CreateTrip: %w", err)
	}
